...
var _clock = require("./js/clock");

var _clock2 = _interopRequireDefault(_clock);

var _command = require("./js/command");
...
var cmdContainer = document.getElementsByClassName("command")[0];
var modal = (0, _modal2.default)(cmdContainer);
var cmd = (0, _command2.default)("/", modal).register("help", function () {
    return console.log("HELP");
});

var container = document.getElementById("container");
var clockClass = (0, _clock.createClockClass)("clock");
(0, _clock2.default)(container, clockClass, cmd);

},{"./js/clock":4,"./js/command":5,"./js/modal":8}],4:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
    value: true
});
exports.createClockClass = undefined;

var _cssClass = require("./cssClass");
...
var addRotatedLine = function addRotatedLine(parent, degree) {
    var attributes = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

    attributes.x1 = attributes.x2 = 0;
    return parent.line().attr(attributes).transform("r" + degree);
};

var addTicks = function addTicks(origin, tickCount, cssClass) {
    var rotationDegree = 360 / tickCount;
    return Array.apply(null, { length: tickCount }).map(function (d, i) {
        return addRotatedLine(origin, rotationDegree * i, { class: cssClass });
    }, Number);
};

var calculateRotation = function calculateRotation(type, value) {
    return { seconds: 360 / 60, minute: 360 / 60, hour: 360 / 12 }[type] * value;
};

var addHands = function addHands(origin, types, now, clockClass) {
    var time = {};
    var milliseconds = now.getMilliseconds();
    time.seconds = now.getSeconds() + milliseconds / 1000, time.minute = now.getMinutes() + time.seconds / 60, time.hour = now.getHours() + time.minute / 60;

    return types.reduce(function (m, d) {
        var handClass = clockClass.get("hands");
        var handGroupClass = clockClass.addModifier(d, "hands");
        m[d] = addRotatedLine(origin.g().attr("class", handGroupClass), calculateRotation(d, time[d]), { class: handClass });
        return m;
    }, {});
};

var initialiseClockElements = function initialiseClockElements(parent, now, clockClass) {
    var origin = parent.g().attr("class", clockClass.get("base"));
    var clockRim = origin.circle().attr("class", clockClass.get("rim"));
    var tickClass = clockClass.get("ticks");
    var ticks = {
        minor: addTicks(origin, 60, tickClass),
        major: addTicks(origin, 12, tickClass)
    };
    var hands = addHands(origin, ["hour", "minute", "seconds"], now, clockClass);
    return { origin: origin, clockRim: clockRim, ticks: ticks, hands: hands };
};

var positionLines = function positionLines(lines, lineOrigin, lineSize) {
...
var drawClock = function drawClock(svg, elements, sizes) {
    var radius = sizes.radius;
    var scaled = sizes.scaled;
    ...
    ["ticks", "hands"].map(function (type) {
        return Object.keys(elements[type]).map(function (lineType) {
            return positionLines(elements[type][lineType], scaled[type].y0, scaled[type][lineType]);
        });
    });
};

var toggleGlitch = function toggleGlitch(svg, elements, clockClass, sizes) {
    var radius = sizes.radius;
    var glitchOffset = sizes.glitchOffset;
    var glitchShadowHeight = sizes.glitchShadowHeight;

    // document.getElementsByTagName("svg")[0].setAttribute("class", "clock-glitch");
    // const glitchGroup = svg.g().attr("class", "clock-glitch");

    var clockShadowCyanClass = createClockClass(clockClass.get("base") + " " + clockClass.addModifier("glitch-cyan"));
    var elementsCloneCyan = initialiseClockElements(svg, new Date(), clockShadowCyanClass);
    var clipPathCyan = elementsCloneCyan.origin.rect().attr({
        "x": -radius.paper,
    ...
    var clockShadowRedClass = createClockClass(clockClass.get("base") + " " + clockClass.addModifier("glitch-red"));
    var elementsCloneRed = initialiseClockElements(svg, new Date(), clockShadowRedClass);
    var clipPathRed = elementsCloneRed.origin.rect().attr({
        "x": -radius.paper,
    ...
    var cloneClockClass = createClockClass(clockClass.get("base") + " " + clockClass.addModifier("glitch-clone"));
    var elementsClone = initialiseClockElements(svg, new Date(), cloneClockClass);
    ...
    (function frame() {
        ...
        filter.select("feTurbulence").attr("baseFrequency", "0, " + (0, _utils.random)(0.01, 0.8));
        filter.select("feOffset").attr({
            "dx": flag * (0, _utils.random)(0, glitchOffset.x),
            "dy": flag * (0, _utils.random)(0, glitchOffset.y)
        });
        filter.attr("width", (0, _utils.random)(90, 100) + "%");
        clipPathCyan.attr("y", (0, _utils.random)(-radius.paper, radius.paper));
        clipPathRed.attr("y", (0, _utils.random)(-radius.paper, radius.paper));
        blinkOriginalClock();
...
var createClock = function createClock(container, clockClass, cmd) {
    var now = new Date(),
        svg = (0, _snapsvg2.default)(".svg"),
    ...
    sizes.scaled = (0, _scaling2.default)(sizes.radius.clock);
    sizes.radius.paper = sizes.radius.clock + Math.max(sizes.glitchOffset.x, sizes.glitchOffset.y) + sizes.scaled.rim.width * 0.5;

    drawClock(svg, elements, sizes);

    var stopGlitch;
    var _toggleGlitch = function _toggleGlitch() {
...
function createClockClass(baseClass) {
    return (0, _cssClass2.default)(baseClass).register("rim", { type: "element", suffix: "rim" }).register("ticks", { type: "element", suffix: "tick" }).register("hands", { type: "element", suffix: "hand" });
}

exports.default = createClock;
exports.createClockClass = createClockClass;

},{"./cssClass":6,"./scaling":9,"./utils":10,"snapsvg":2}],5:[function(require,module,exports){
"use strict";
...
var _keyboardEventPolyfill = require("./keyboardEventPolyfill");

var _keyboardEventPolyfill2 = _interopRequireDefault(_keyboardEventPolyfill);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

(0, _keyboardEventPolyfill2.default)();

var addListener = function addListener(element, handler) {
    element.addEventListener("keydown", handler);
};

var removeListener = function removeListener(element, handler) {
    element.removeEventListener("keydown", handler);
};

var command = function command(triggerKey, modal) {
    var _ref = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

    var _ref$className = _ref.className;
    var className = _ref$className === undefined ? "command" : _ref$className;

    // let props = {className};
    var commands = {
        fallback: function fallback(command, modal) {
            var commands = api.list().filter(function (cmd) {
                return cmd !== "fallback";
            });
            modal.message = ["" + triggerKey + command + " is invalid :'(", "try one of " + triggerKey + commands.join(", " + triggerKey)].join(" ");
    ...
    var api = {};
    api.register = function (commandString, cb) {
        commands[commandString] = cb;
        return api;
    };
    api.remove = function (commandString) {
        delete commands[commandString];
        return api;
    };
    api.trigger = function (commandString) {
        ...
        if (api.list().indexOf(commandString) < 0) {
            ...
            commands[commandString].apply(commands, [commandString].concat(args));
            modal.kill();
        }
        return api;
    };
...
},{"./keyboardEventPolyfill":7}],6:[function(require,module,exports){
"use strict";
...
var setBase = function setBase(baseName) {
    if (baseName === "") {
    ...
    var constructClass = function constructClass(type, suffix, base) {
        var _base = base.split(" ");
        var _construct = function _construct(type, suffix, base) {
            return base + cssClassSymbols[type] + suffix;
        };
        return [].concat(_base).map(function (d) {
            return _construct(type, suffix, d);
        }).join(" ");
    };
    ...
    var api = {
        ...
        register: function register(name) {
            var _ref = arguments.length <= 1 || arguments[1] === undefined ? { type: null, base: null } : arguments[1];

            var type = _ref.type;
            var suffix = _ref.suffix;
            var base = _ref.base;

            var baseClass = getClass(base);
            var newClass = constructClass(type, suffix, baseClass);
            classNames[name] = newClass;
            return api;
        },
        addElement: function addElement(suffix) {
            var base = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];

            var baseClass = getClass(base);
            return constructClass("element", suffix, baseClass);
        },
        addModifier: function addModifier(suffix) {
            var base = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
...
for (var _i = 65; _i <= 90; _i++) {
    var letter = String.fromCharCode(_i).toLowerCase();
    keys[_i] = [letter, letter.toUpperCase()];
}
...
var createModal = function createModal(container) {
    var elements = {
